...
def train(
    ...
    tracker = TrainingTracker(
        writer=writer, log_file=str(save_dir / "train.log"), rank=accelerator.rank
    )
    ...
    if max_batch_tokens and max_batch_tokens > 0:
        ...
        audio_vae_fps = (
            base_model.audio_vae.sample_rate / base_model.audio_vae.hop_length
        )
        ...
        max_sample_len = (
            max_batch_tokens // batch_size if batch_size > 0 else max(est_lengths)
        )
    ...
    def _signal_handler(
        ...
        try:
            save_checkpoint(
                _model, _optim, _sched, _save_dir, cur_step, _pretrained, _hf_id, _dist
            )
    ...
    with tracker.live():
        for step in range(start_step, num_iters):
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                unwrapped_model.parameters(), max_norm=1e9
            )
            ...
            if step % log_interval == 0:
                ...
                epoch = (step * grad_accum_steps * batch_size) / max(
                    1, num_train_samples
                )
            ...
            if val_loader is not None and step % valid_interval == 0 and step != 0:
                validate(
                    model, val_loader, batch_processor, accelerator, tracker, lambdas
                )
...
def save_checkpoint(
    ...
    if lora_cfg is not None:
        ...
        base_model_to_save = (
            hf_model_id
            if distribute
            else (str(pretrained_path) if pretrained_path else None)
        )
        lora_info = {
            "base_model": base_model_to_save,
            "lora_config": lora_cfg.model_dump()
            if hasattr(lora_cfg, "model_dump")
            else vars(lora_cfg),
        }
